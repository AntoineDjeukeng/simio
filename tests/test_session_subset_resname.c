...
int main(void)
{
    ...
    ft_session_set_path_policy(S, &(t_path_policy){ .subset_suffix="_res", .preserve_ext=1 });

    const char *drop[] = { "WAL" };
    t_subset_spec sub = { .mode=SUBSET_BY_RESNAME, .wall_resnames=drop, .n_resn=1 };
    ...
    rc = gro_read(p, &T, &F);
    assert(rc == FT_OK);
    assert(T.natoms == 4); /* 6 total - 2 WAL atoms = 4 */
    puts("test_session_subset_resname OK");
