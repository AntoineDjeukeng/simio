...
GatingCenterPlaneProperty::GatingCenterPlaneProperty(const GatingCenterPlaneConfig& cfg) : cfg_(cfg) {}

const char* GatingCenterPlaneProperty::name() const { return "GatingCenterPlaneProperty"; }

uint64_t GatingCenterPlaneProperty::requires() const {
...
void GatingCenterPlaneProperty::compute_frame(const layered::FrameContext& ctx) {
    ...
    for (size_t mid = 0; mid < nmol; ++mid) {
        ...
        if (has_prev_[mid] && include) {
            ...
            const CrossingResult crossing =
                detect_crossing(key_prev.v[0], dr_mi.v[0], ctx.layer0.x_center, ctx.frame.pbc.L[0]);
...
void GatingCenterPlaneProperty::finalize() {}

const std::vector<GatingFrameCount>& GatingCenterPlaneProperty::frames() const { return frames_; }

const GatingFrameCount& GatingCenterPlaneProperty::cumulative() const { return cumulative_; }
